...
export function buildPostCompletionReviewSnapshot(input: {
  packageId: string;
  packageName?: string | null;
  ...
  preset?: {
    postCompletionReviewPromptSnapshot?: Record<string, unknown> | null;
  } | null;
}) {
  const reviewedAt =
    input.reviewedAt instanceof Date
      ? input.reviewedAt.toISOString()
      : (input.reviewedAt as string | null | undefined) ?? null;
  const retainedWarnings = (input.warningSnapshot ?? []).map((warning) => warning.message);
  return {
    reviewVersion: "post-completion-review-v1",
    packageIdentity: {
      packageId: input.packageId,
      packageName: input.packageName ?? null
    },
    ...
    retainedWarnings,
    retainedOverrideSummary: input.overrideSnapshot ?? null,
    completedArtifactSummary: input.completedArtifactSummarySnapshot ?? null,
    prompts: input.preset?.postCompletionReviewPromptSnapshot ?? null,
    summary:
      input.reviewNote && input.reviewNote.trim().length > 0
        ? `Post-completion review recorded for ${input.packageName ?? input.packageId}.`
        : "Post-completion review recorded."
  };
}

export function buildArtifactSupersessionSummarySnapshot(input: {
  packageId: string;
  packageName?: string | null;
  currentApprovedArtifact: boolean;
  entryCompletionConfirmed: boolean;
  ...
  supersedingPackage?: {
    ...
    currentApprovedArtifact?: boolean | null;
    updatedAt?: string | Date | null;
  } | null;
  preset?: {
    supersessionSummaryFormatSnapshot?: Record<string, unknown> | null;
  } | null;
}) {
  const status =
    input.entryCompletionConfirmed && input.currentApprovedArtifact
      ? "CURRENT_COMPLETED"
      : input.entryCompletionConfirmed && input.supersedingPackage
  ...
  const supersededAt =
    input.supersedingPackage?.updatedAt instanceof Date
      ? input.supersedingPackage.updatedAt.toISOString()
      : (input.supersedingPackage?.updatedAt as string | null | undefined) ?? null;

  return {
    supersessionVersion: "supersession-v1",
    artifactSupersessionStatus: status,
    supersededAt,
    supersededByListingPrepPackageId: input.supersedingPackage?.id ?? null,
    replacementArtifact: input.supersedingPackage
      ? {
          id: input.supersedingPackage.id,
          name: input.supersedingPackage.name ?? null,
          approvalState: input.supersedingPackage.approvalState ?? null,
          currentApprovedArtifact: Boolean(input.supersedingPackage.currentApprovedArtifact)
        }
      : null,
    explanation:
      status === "CURRENT_COMPLETED"
        ? "This completed artifact is still the current source of truth."
        : status === "SUPERSEDED_COMPLETED"
          ? "A newer approved artifact has replaced this completed artifact."
          : status === "COMPLETED_NO_REPLACEMENT"
            ? "This artifact was completed and no newer replacement is currently active."
            : "This artifact is historical and was not finalized as the current completed source of truth.",
    retainedSummary:
      input.packageName
        ? `${input.packageName} remains retained as historical reference.`
        : "This artifact remains retained as historical reference.",
    channelNotes: input.preset?.supersessionSummaryFormatSnapshot ?? null
  };
}
