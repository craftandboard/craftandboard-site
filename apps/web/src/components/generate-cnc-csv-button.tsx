...
export function GenerateCncCsvButton(props: { batchId: string }) {
  ...
  async function handleGenerate() {
    ...
    try {
      const payload = await generateBatchCncCsv(props.batchId);
      setResult(payload);
      startTransition(() => router.refresh());
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : "CNC CSV generation failed.";
      setError(message);
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {pending ? "Generating CNC CSV..." : "Generate CNC CSV"}
      </button>
      {result && result.ok ? (
        <a
