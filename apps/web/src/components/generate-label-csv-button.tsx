...
import { generateBatchLabelCsv, type BatchExportArtifactResponse } from "../lib/api";
import { API_BASE_URL } from "../lib/site-config";

export function GenerateLabelCsvButton(props: { batchId: string }) {
  ...
  async function handleGenerate() {
    ...
    try {
      const payload = await generateBatchLabelCsv(props.batchId);
      setResult(payload);
      startTransition(() => router.refresh());
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : "Label CSV generation failed.";
      setError(message);
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {pending ? "Generating Label CSV..." : "Generate Label CSV"}
      </button>
      {result && result.ok ? (
        <a
