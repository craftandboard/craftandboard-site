...
import { type FloatingMantelConfig } from "../../lib/storefront/floatingMantel";
import {
  buildFloatingMantelInquiryHref,
  buildFloatingMantelPdpHref,
  type FloatingMantelOrderDraft,
...
export function FloatingMantelCheckout({
  initialConfiguration
}: {
  initialConfiguration: FloatingMantelConfig;
}) {
  ...
  useEffect(() => {
    ...
    void getFloatingMantelPricing({ configuration: initialConfiguration })
      .then((payload) => {
        if (!active) {
          return;
        }
  ...
  useEffect(() => {
    ...
    void getCraftBoardStorefrontQuote({
      ...
      .then((payload) => {
        ...
        setPricing(payload.quote.pricing as FloatingMantelPricingResult);
      })
      .catch((caught) => {
        if (!active) {
          return;
  ...
  const inquiryHref = useMemo(
    () => buildFloatingMantelInquiryHref(initialConfiguration, "/order/floating-mantels/classic-floating-mantel"),
    [initialConfiguration]
  );

  const editHref = useMemo(
    () => buildFloatingMantelPdpHref(initialConfiguration),
    [initialConfiguration]
  );
  ...
  function handleSubmit() {
    ...
    const draft: FloatingMantelOrderDraft = {
    ...
    startTransition(() => {
      void submitCraftBoardStorefrontOrder({
        sourcePath: resolveStorefrontSourcePath("/order/floating-mantels/classic-floating-mantel"),
        draft
      })
        .then((payload) => {
  ...
  return (
    <div className="space-y-6">
      <ProductCheckoutSummary
        productTitle="Classic Floating Mantel"
        configurationLines={[
          `Length: ${initialConfiguration.length} in`,
          `Depth: ${initialConfiguration.depth} in`,
          `Height: ${initialConfiguration.height} in`,
          `Material: ${initialConfiguration.materialLabel}`,
          `Support: ${initialConfiguration.mountingLabel}`,
      ...
      <section className="rounded-[2rem] border border-[#dbcab9] bg-[#fffaf4] p-6">
        ...
        <textarea
          ...
          placeholder="Optional fireplace wall, stone surround, or delivery notes"
        ...
        <div className="mt-5 space-y-3 text-sm text-[#4f3f33]">
          ...
          <label className="flex items-start gap-3">
            ...
            <span>I acknowledge the length, depth, height, material, support path, and pricing basis shown above.</span>
          </label>
          <label className="flex items-start gap-3">
